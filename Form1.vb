...
Public Class Form1
    Private version As String = "TvRemoteViewer_VB version 0.02"
    ...
    Private Sub ButtonMovieStart_Click(sender As System.Object, e As System.EventArgs) Handles ButtonMovieStart.Click
        If Me._worker._isWebStart = True Then
            ...
            If bondriver.IndexOf(".dll") > 0 Then
                If s.Length = 3 Then
                    sid = Val(s(1))
                    Me._worker.start_movie(num, bondriver, sid, chspace, udpApp, hlsApp, hlsOpt1, hlsOpt2, wwwroot, fileroot, hlsroot)
                Else
    ...
    Private Sub StartHttpServer()
        ...
        Me._worker = New WebRemocon(udpApp, udpPort, chSpace, hlsApp, hlsOpt1, hlsOpt2, wwwroot, fileroot, wwwport, BonDriverPath)
    ...
    'フルパスからフォルダを取り出して返す
    Private Function filepath2path(ByVal s As String, Optional ByVal addstr As String = "") As String
        Dim r As String
